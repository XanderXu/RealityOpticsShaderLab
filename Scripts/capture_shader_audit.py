...
def main():
    ...
    parser.add_argument("--performance", action="store_true", help="Check lazy caches and instances instead of screenshots")
    args = parser.parse_args()
    if args.performance and args.effect:
        parser.error("--performance starts with thinFilm; omit --effect")
    args.output.mkdir(parents=True, exist_ok=True)
    bundle = "com.reality.RealityOpticsShaderLab"
    env = dict(os.environ)
    flag = "SIMCTL_CHILD_OPTICS_PERF_AUDIT" if args.performance else "SIMCTL_CHILD_OPTICS_AUDIT"
    env[flag] = "1"
    marker = "OPTICS_PERF_AUDIT" if args.performance else "OPTICS_AUDIT"
    if args.effect:
    ...
    try:
        with (args.output / "runtime.log").open("w") as log:
            for line in process.stdout:
                log.write(line)
                log.flush()
                if "OPTICS_AUDIT" in line or "OPTICS_PERF" in line:
    ...
    expected = 0 if args.performance else (1 if args.effect else 16)
