...
> 后续效果扩展至 28 项/22 个运行时模板，新增 4 张常规固定 LUT（复查后合并二向色 R/T）；完整访问后的缓存上界为 13 张、约 6.26 MiB 像素负载。启动仍仅加载选中项。下文的 16 项测量是扩展前的基线；新结果与模型边界见 [新增效果说明](EXTENDED_EFFECTS.md)。
...
后续预览更新改为同时显示两件模型，默认球体＋平面，切换为尺子＋光盘。两件模型共享同一个材质定义和 LUT；主要复用收益来自模板、LUT 与四种网格资源。没有为两件不同几何的预览引入 visionOS 26 专属的批量实例 API。

额外优化包括：缓存参数 handle，跳过值相同的参数写入；暂停时只在新实体出现后设置一次姿态；保持两个活动实体，复用四种网格。新加入的材质基础色使用线性 sRGB 参数和一个 mix 节点，不增加 LUT 或纹理；分组/底色切换不会预热其他效果。上方启动测量记录于双模型更新之前。生物结构色沿用之前注明的视觉近似。视线/法线决定的颜色仍实时计算；没有把随观察角变化的结果烘焙成固定贴图。
