...
所有效果共享四几何体预览（visionOS 与 iOS）、原始／黑白及 RGB 基础色、底色替换比例与效果强度。基础色替换独立体色或 LUT 的中性成分，保留彩色差值与独立高光；玻璃改变体吸收，星云底色遵循原有层间遮挡。RGB 中性成分分离属于外观近似，不推断真实有色基底的光谱。光源方位属于解析演示光源，并不读取房间中的灯光。
