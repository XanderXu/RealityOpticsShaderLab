...
- 预览水平外边距为 4 pt，模型中心列距 0.29 m、行距 0.26 m。相机按各网格的旋转包围球适配视野，减少四周空白；与 visionOS 共用四模型布局尺寸。
- 下半屏默认显示效果库／参数入口、基础色控制和直接展开的效果说明。说明随所选效果更新，不再放在参数面板中。打开底部面板后，下半屏切换为效果库或参数；关闭按钮或下拉顶部拖动条可收起，上方预览始终保留。
...
四模型回归通过：35 种效果、5 种基础色、3 种比例，所有场景保持 4 个模型；切换颜色与旧分组属性未新增材质模板或纹理。移动端专项检查和四模型回归各保存 7 张截图。基础色现使用“底色替换”：默认 100%，保留光学分量；0% 恢复原始效果。

紧凑布局更新：visionOS 也改为四模型 2×2，移除分组切换；两端统一收紧模型间距。新版 [iPhone 横竖屏与旋转检查](../artifacts/compact-mobile-preview/) 和 [Vision Pro 四模型回归](../artifacts/compact-vision-preview-final/) 均通过，各保存 7 张截图；两端 Debug [构建日志](../artifacts/compact-layout-verification/)。
